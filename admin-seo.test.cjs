...
(async()=>{try{
 ...
 const health=await(await fetch(url+'/api/health')).json();assert.equal(health.version,'2026-09-12-review-2');
 const initial=await fetch(url+'/');assert(initial.headers.get('content-security-policy').includes("script-src 'self'"));
 ...
 await api('/api/page-content','PUT',{home:{heroTitle:'AI automation for the work behind your business.'}});
 html=await(await fetch(url+'/')).text();assert(html.includes('<h1>Stop answering the same WhatsApp questions all day.</h1>'),'Persisted legacy defaults must upgrade on read');
 ...
 await api('/api/collections/services/'+svc.id,'PUT',{...after.services[0],seoTitle:'QA independent search title',seoDescription:'QA independent search description'});
 let seoPages=structuredClone(data.seo.pages);delete seoPages['/services/'+svc.slug];
 await api('/api/seo','PUT',{pages:seoPages});
 html=await(await fetch(url+'/services/'+svc.slug)).text();assert(html.includes('<title>QA independent search title</title>'));assert(html.includes('content="QA independent search description"'));
 const contactHtml=await(await fetch(url+'/contact')).text();assert(contactHtml.includes('<option>Under 100</option>'));
