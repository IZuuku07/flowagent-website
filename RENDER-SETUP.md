...
4. Configure ADMIN_USERNAME=admin, a new ADMIN_PASSWORD (at least 16 characters), and a generated SESSION_SECRET privately before removing the exposed server-config.json. Do not reuse the publicly exposed secret. Deploy the prepared update only after credentials and database access are ready.
